...
from custom_nodes.ComfyUI_InvSR.InvSR.sampler_invsr import InvSamplerSR, BaseSampler

from custom_nodes.ComfyUI_InvSR.InvSR.utils import util_common
from custom_nodes.ComfyUI_InvSR.InvSR.utils.util_opts import str2bool
from custom_nodes.ComfyUI_InvSR.InvSR.basicsr.utils.download_util import load_file_from_url
...
def get_configs(args, log=False):
    ...
    if args.started_ckpt_path:
        started_ckpt_dir = "custom_nodes/ComfyUI_InvSR/weights"
...
def main():
    ...
    # Load model node
    args = Namespace(in_path='./testdata/RealSet80/9.jpg', out_path='outputs', bs=1, chopping_bs=8, timesteps=None, num_steps=1, cfg_path='./configs/sample-sd-turbo.yaml', sd_path='', started_ckpt_path='', tiled_vae=True, color_fix='', chopping_size=128)
    configs = get_configs(args)
    base_sampler = BaseSampler(configs)

    # Sampling node
    args = Namespace(in_path='/workspace/ComfyUI/custom_nodes/ComfyUI_InvSR/InvSR/testdata/RealSet80/23.jpg', out_path='/workspace', bs=1, chopping_bs=8, timesteps=None, num_steps=1, cfg_path='./configs/sample-sd-turbo.yaml', sd_path='', started_ckpt_path='/workspace/ComfyUI/custom_nodes/ComfyUI_InvSR/weights/noise_predictor_sd_turbo_v5.pth', tiled_vae=True, color_fix='', chopping_size=256)
    sampler = InvSamplerSR(base_sampler, configs)
    sampler.configs = get_configs(args)
    sampler.setup_seed(3423423)
